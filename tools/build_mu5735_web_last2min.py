...
def build() -> dict:
    ...
    payload = {
        "meta": {
            ...
            "eyebrow": f"基于 ADS-B Reveal 与 FDR 重叠段的 {label}",
            "subtitle": f"本窗口以 ADS-B 时间轴组织回放。窗口前约 {fdr_overlap_seconds:.1f} 秒与 FDR 记录重叠；其后约 {fdr_gap_after_end:.1f} 秒仅保留 ADS-B 可支持的外部运动信息。",
            ...
            "windowSummary": f"按 ADS-B 末条记录回溯 {window_label(VIS_DURATION).replace('最后 ', '')}；前段存在有限 FDR 重叠，后段以 ADS-B 记录为主",
            ...
            "primaryEventText": f"前段 FDR 重叠约 {fdr_overlap_seconds:.1f}s，后段以 ADS-B 为主",
            "summaryFuelText": f"发动机相关 FDR 记录覆盖前约 {fdr_overlap_seconds:.1f}s",
            "snapshotMode": "mixed",
            "noDataMessages": {
                "chart-attitude": "该时段缺少 FDR 姿态记录。",
                "chart-controls": "本窗口无 FDR 操纵记录。",
                "chart-engines": "本窗口无 FDR 发动机转速记录。",
                "chart-fuel": "本窗口无 FDR 燃油流量记录。",
            },
            "notes": [
                f"该 {window_label(VIS_DURATION).replace('最后 ', '')}窗口取自 ADS-B Reveal 最后一条记录向前回溯。",
                f"窗口前约 {fdr_overlap_seconds:.1f} 秒与 FDR 重叠，后约 {fdr_gap_after_end:.1f} 秒晚于 FDR 终止，因此后段不再包含姿态、操纵与发动机的 FDR 记录。",
                "高度、地速、航向、经纬度与垂直速度采用 ADS-B 数据，并在相邻点之间做时间插值以便连续回放。",
                "轨迹角 = atan2(垂直速度, 地速)，它描述飞行路径倾角，不等同于机体俯仰角。",
                "航向变化率由 ADS 航向差分得到，它不等同于机体偏航率。",
                f"当前窗口内可用 FDR 样本计数: {overlap_samples}。",
            ],
        },
        "anchors": anchors,
        "frames": frames,
